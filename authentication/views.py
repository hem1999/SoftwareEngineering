...
def add_projects(request , authorName):
    if request.method=="POST":
        ...
        proj.save()
        obj = Project_user(user_id = request.user ,project_id =proj)
        obj.save()
        return redirect('dashboard')
    else:
        try:
...
def view_projects(request):
    ...
    proj = Project_user.objects.filter(user_id = prof[0] )
    l=[]
...
def add_projects(request):
    ...
    if request.method=="POST":
        ...
        pu =Project_user(user_id =prof , project_id = proj)
        pu.save()
...
def requests(request):
    if request.method == "POST":
        ...
        user = request.user
        prof = Professor.objects.get(name = user.username )
        req = Join_team.objects.filter(to_user = prof.id , status= False )
